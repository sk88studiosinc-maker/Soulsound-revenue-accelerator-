...
export const analyzeSiteRevenue = async (url: string): Promise<SiteAnalysis> => {
  const response = await ai.models.generateContent({
    ...
    contents: `Complete forensic audit of all accounts associated with:
    - sk88studiozinc@gmail.com
    - soulsoundworld.live@gmail.com
    - sk88studiosinc@gmail.com
    
    Target Ecosystem:
    - Artist: Samzin Kreave (Samzin)
    - Labels: SoulSoundWorld.world, distributed via Fresh Tunes.
    - Platforms: Spotify, Apple Music, iTunes, Pocket FM (Stories: 'Tomorrows Yesterday', 'Joker for the Queen'), Zora (SoulSound).

    Primary Directive: Use "Metric Logic" to identify passive, active, and potential income. Find every penny left on the table.
    ...
    3. 'businessAdjustments': Strategic shifts to improve overall revenue (e.g., pivot content types, tiered subscription loops, etc.).
    4. Exact metadata triggers for monitoring these specific emails for hidden royalty receipts.
...
export const analyzeRoyaltyMetadata = async (metadataBatch: string[]): Promise<RoyaltyInsight[]> => {
  const response = await ai.models.generateContent({
    ...
    Identify hidden cash in emails: sk88studiozinc, soulsoundworld.live, sk88studiosinc.
